...
class Swift3HelperTestCase(unittest.TestCase):
    ...
    def test_fake_swift_sysmeta(self):
        ...
        swift.register(method, path, MagicMock(), orig_headers, None)

        self._check_headers(swift, method, path, orig_headers)

        new_headers = orig_headers.copy()
        del new_headers[sysmeta_header('container', 'acl').title()]
        swift.register(method, path, MagicMock(), new_headers, None)
    ...
    def test_fake_swift_sysmeta_overrite(self):
        ...
        self._check_headers(swift, method, path, new_headers)
