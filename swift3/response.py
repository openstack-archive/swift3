...
class S3DateTime(datetime):
    ...
    @classmethod
    def fromtimestamp(klass, timestamp, tz_info=swob.UTC):
        dt = super(S3DateTime, klass).fromtimestamp(timestamp, tz_info)
        # drop microsecond info
        return klass(*(dt.timetuple()[:6] + (0, tz_info,)))

    @property
    def s3xmlformat(self):
        isoformat = self.isoformat()
        if self.utcoffset() is not None:
            isoformat = isoformat[:-6]
