...
class SigV4Mixin(object):
    ...
    def _parse_header_authentication(self):
        ...
        sh_str = auth_str.partition("SignedHeaders=")[2].split(',', 1)[0]
        # credential must be in following format:
        # <access-key-id>/<date>/<AWS-region>/<AWS-service>/aws4_request
    ...
    def _headers_to_sign(self):
        ...
        for h in self._signed_headers.split(';'):
...
class Request(swob.Request):
    ...
    def timestamp(self):
        if not self._timestamp:
            if self._is_query_auth:
                self._timestamp = S3Timestamp(
                    mktime(self.params.get('Timestamp'),
                           SIGV2_TIMESTAMP_FORMAT))
            elif self._is_header_auth:
                timestamp = mktime(
    ...
    def _validate_date_param(self, now):
        ...
        # TODO: make sure if signed URL also needs TimeSkewed check?
