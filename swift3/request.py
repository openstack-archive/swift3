...
class Request(swob.Request):
    ...
    def get_response(self, app, method=None, container=None, obj=None,
                     headers=None, body=None, query=None, resource=None,
                     permission=None):
        """
        Calls the application with this request's environment.  Returns a
        Response object that wraps up the application's result.
        ...
        def _check_permission():
            if resource == 'OBJECT':
                o_resp = self._get_response(app, 'HEAD', container, obj)
                if permission == 'OWNER':
                    o_resp.object_acl.check_owner(self.user_id)
                else:
                    o_resp.object_acl.check_permission(self.user_id,
                                                       permission)
                return o_resp
            else:
                c_resp = self._get_response(app, 'HEAD', container, None)
                if permission == 'OWNER':
                    c_resp.bucket_acl.check_owner(self.user_id)
                else:
                    c_resp.bucket_acl.check_permission(self.user_id,
                                                       permission)
                return c_resp
        ...
        if CONF.s3_acl and not (permission == 'NO_CHECK'):
            ...
            if (s3_method, sw_method, sw_resource) in ACL_MAP:
                ...
                resource = resource or acl_check['Resource']
                permission = permission or acl_check['Permission']

            if not (resource and permission):
                # If value doesn't set, not check permission.
                return self._get_response(app, sw_method, container, obj,
                                          headers, body, query)

            # If the request to swift is HEAD and the resource is consistent
            # with the confirmation subject of ACL, not request again.
            # This is because that contains the information required in the
            # HEAD response at the time of ACL acquisition.
