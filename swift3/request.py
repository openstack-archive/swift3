...
# TODO: apply SIGV2_TIMESTMP_FORMAT for query auth
SIGV2_TIMESTAMP_FORMAT = '%Y-%m-%dT%H%M%SZ'
...
def signature_v4(req_cls):
    ...
    class SigV4Class(req_cls):
        ...
        def _is_query_auth(self):
            return True if 'X-Amz-Credential' in self.params else False
        ...
        def timestamp(self):
            ...
            try:
                if self._is_query_auth:
                    ...
                    # format
                    timestamp = mktime(
                        self.params.get('X-Amz-Date'), SIGV4_X_AMZ_DATE_FORMAT)
            ...
            # TODO: make sure this is needed or not.
            if timestamp is None:
                return None

            return S3Timestamp(timestamp)

        def _validate_date_param(self, now):
            """
            :param now: a S3Timestamp instance
            """
            expires = self.params['X-Amz-Expires']
            if int(self.timestamp) + int(expires) < now:
                raise AccessDenied('Request has expired')

        def _validate_date_header(self, now):
            epoch = S3Timestamp(0)
            ...
            # error
            delta = 60 * 5
            if abs(int(self.timestamp) - int(now)) > delta:
        ...
        def _parse_query_authentication(self):
            ...
            # http://doc.aws.amazon.com/AmazonS3/latest/API/
            # sigv4-auth-using-authorization-header.html
            # it seems we need to verify the all incomming header keys
            # maches with signed_headers list.
        ...
        def _headers_to_sign(self):
            ...
            """
            headers_lower = dict(
                (k.lower().strip(), ' '.join(v.strip().split()))
                for (k, v) in six.iteritems(self.headers) if v)
            ...
            for h in self.signed_headers.split(';'):
        ...
        def _build_signed_headers(self, headers_to_sign):
            return ';'.join(sorted(
                n.lower().strip()
        ...
        def _string_to_sign(self):
            ...
            cr.append(self._build_signed_headers(headers_to_sign))
...
class Request(swob.Request):
    ...
    def timestamp(self):
        if self._is_query_auth:
            return S3Timestamp(
                mktime(self.params.get('Timestamp'), SIGV2_TIMESTAMP_FORMAT))
        elif self._is_header_auth:
            timestamp = mktime(
                self.headers.get('X-Amz-Date', self.headers.get('Date')))
            return S3Timestamp(timestamp)

    @property
    def _is_header_auth(self):
        return True if 'Authorization' in self.headers else False

    @property
    def _is_query_auth(self):
        return True if 'AWSAccessKeyId' in self.params else False
    ...
    def _validate_date_param(self, now):
        ...
        # TODO: make sure if singed URL also needs TimeSkewed check?
    ...
    def _validate_dates(self):
        ...
        now = S3Timestamp.now()
        if self._is_query_auth:
            self._validate_date_param(now)
        else:
            # NOTE: this should be header auth.
            self._validate_date_header(now)
