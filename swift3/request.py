...
def signature_v4(req_cls):
    ...
    class SigV4Class(req_cls):
        ...
        def _validate_date_param(self, now):
            ...
            if date + datetime.timedelta(seconds=int(expires)) < now:
        ...
        def _validate_date_header(self, now):
            ...
            if self.headers.get('X-Amz-Date'):
                try:
                    date = datetime.datetime.strptime(
                        self.headers['X-Amz-Date'], SIGV4_X_AMZ_DATE_FORMAT)
                except ValueError:
                    # TODO: Can we try Date header or raise something
                    # immedeately?
        ...
        def _string_to_sign(self):
            ...
            """
            scope = (self.timestamp.split('T')[0] +
...
class Request(swob.Request):
    ...
    def timestamp(self):
        if self._is_query_auth:
            return self.params.get('X-Amz-Date')
        elif self._is_header_auth:
            return self.headers.get('X-Amz-Date')
    ...
    def _validate_date_header(self, now):
        date_header = self.headers.get('X-Amz-Date', self.headers.get('Date'))
