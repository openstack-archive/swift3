...
from swift3.response import HTTPOk, NoSuchBucket, BucketAlreadyExists, \
    BucketAlreadyOwnedByYou, MalformedXML
...
class VersioningController(Controller):
    ...
    def PUT(self, req):
        ...
        versions_container = ''.join([req.container_name, VERSIONING_SUFFIX])
        req.container_name += VERSIONING_SUFFIX
        try:
            req.get_response(self.app, 'PUT', versions_container, '')
        ...
        if status == 'Suspended':
            req.headers.pop('X-History-Location')
